...
public class DatabaseHelper extends SQLiteOpenHelper
{
    ...
    public static final String veritabaniAdi = "notlarVeritabani.db";
    static final int veritabaniVersiyon = 1;
    static final String tabloAdi = "notlar";
    static final String kolonId = "notid";
    static final String kolonAdi = "notIcerik";

    static final String TabloOlusturSql = "CREATE TABLE " + tabloAdi + " (" +
            kolonId + " INTEGER PRIMARY KEY AUTOINCREMENT, "
            + kolonAdi + " Text )";
    ...
    void notEkle(String not)
    {
        ...
        if(sonuc>-1)
            Log.i("Kayit","Not başarıyla kaydedildi.");
        else
            Log.i("Kayit","Not kaydedilmesi.");
        db.close();
    }
    ...
    ArrayList<Notlar> notlariGetir()
    {
        ...
        Cursor c=db.query(tabloAdi,gelecekKolonlar,
                null,null,null,null,null) ;

        while(c.moveToNext())
        {
            data.add(new Notlar(c.getString(c.getColumnIndex(kolonId)),
                    c.getString(c.getColumnIndex(kolonAdi) )));
        }
